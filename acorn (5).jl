...
function run_acorn(
    ...
    storage_eff=0.75,                        # legacy default
    ...
    # Read storage (metadata only; arrays that need nt are built later)
    storage = CSV.read("$(run_directory)/inputs/$(storage_filename)", DataFrame)

    # Helpers
    hascol(df, sym) = sym in names(df)
    n_s = nrow(storage)

    # Optional per-asset columns with defaults
    eff_vec          = hascol(storage, :eff)           ? Float64.(storage.eff)           : fill(storage_eff, n_s)
    init_soc_vec     = hascol(storage, :init_soc)      ? Float64.(storage.init_soc)      : fill(0.30, n_s)
    end_soc_min_vec  = hascol(storage, :end_soc_min)   ? Float64.(storage.end_soc_min)   : fill(0.0,  n_s)
    discharge_cost   = hascol(storage, :discharge_cost_per_mwh) ? Float64.(storage.discharge_cost_per_mwh) : zeros(n_s)
    ...
    #NEW Per-asset SOC settings (optional columns with safe defaults)
    init_soc_vec = in(:init_soc, names(storage)) ?
        Float64.(storage[!, :init_soc]) : fill(0.30, nrow(storage))   # default 30%

    end_soc_min_vec = in(:end_soc_min, names(storage)) ?
        Float64.(storage[!, :end_soc_min]) : fill(0.0, nrow(storage)) # default 0%
    ...
    @objective(model, Min,10000 * sum(load_shedding) + (sum(charge) + sum(discharge)) + sum(gencost .* pg) + 
        sum([discharge_cost[i] * sum(discharge[i, :]) for i in 1:n_s]))
